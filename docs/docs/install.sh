#!/bin/sh
# Blob CLI Installer Script
# Usage: curl -fsSL https://meigma.github.io/blob/install.sh | sh
#
# Environment variables:
#   BLOB_VERSION    - Version to install (default: latest)
#   BLOB_INSTALL    - Installation directory (default: ~/.local/bin)
#   BLOB_TMPDIR     - Parent directory for temporary files (default: system temp)
#                     A subdirectory will be created and cleaned up on exit
#   BLOB_NO_VERIFY  - Skip signature verification if set to "1"
#
# Examples:
#   # Install latest version
#   curl -fsSL https://meigma.github.io/blob/install.sh | sh
#
#   # Install specific version
#   curl -fsSL https://meigma.github.io/blob/install.sh | BLOB_VERSION=1.0.0 sh
#
#   # Install to custom directory
#   curl -fsSL https://meigma.github.io/blob/install.sh | BLOB_INSTALL=/usr/local/bin sh

set -e

# Configuration
GITHUB_OWNER="meigma"
GITHUB_REPO="blob-cli"
BINARY_NAME="blob"
...
COSIGN_CERT_IDENTITY_REGEXP="https://github.com/meigma/blob-cli/.github/workflows/release.yml@refs/tags/.*"
...
get_latest_version() {
    ...
    if [ -z "$version" ]; then
        log_error "Failed to fetch latest version. Please specify BLOB_VERSION."
        exit 1
    fi
...
verify_signature() {
    ...
    if [ "${BLOB_NO_VERIFY:-0}" = "1" ]; then
        log_warn "Signature verification disabled via BLOB_NO_VERIFY"
        return 0
    fi
...
main() {
    log_info "${BOLD}Blob CLI Installer${NC}"
    echo ""
    ...
    install_dir="${BLOB_INSTALL:-$(get_default_install_dir)}"
    log_info "Installation directory: ${install_dir}"

    # Create temp directory
    # If BLOB_TMPDIR is set, create a subdirectory within it to avoid
    # accidentally deleting user data on cleanup
    if [ -n "$BLOB_TMPDIR" ]; then
        tmpdir=$(mktemp -d "${BLOB_TMPDIR}/blob-install.XXXXXX")
    else
        tmpdir=$(mktemp -d)
    fi
