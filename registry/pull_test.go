...
func TestClient_Pull(t *testing.T) {
	...
	t.Run("index cache hit skips fetch blob", func(t *testing.T) {
		...
		testManifest, testManifestBytes, cacheDesc := manifestForIndexData(t, indexData, dataBytes)
		indexDigest := testManifest.Layers[0].Digest.String()
	...
	t.Run("skip cache option bypasses caches", func(t *testing.T) {
		...
		testManifest, testManifestBytes, cacheDesc := manifestForIndexData(t, indexData, dataBytes)
		indexDigest := testManifest.Layers[0].Digest.String()
		testDigestStr := cacheDesc.Digest.String()
		...
		require.NoError(t, refCache.PutDigest(testRef, testDigestStr))
		manifestCache := newMemManifestCache()
		require.NoError(t, manifestCache.PutManifest(testDigestStr, testManifestBytes))
		indexCache := newMemIndexCache()
		require.NoError(t, indexCache.PutIndex(indexDigest, indexData))
	...
	t.Run("blob URL error propagates", func(t *testing.T) {
		...
		indexData, dataBytes := createTestBlobData(t)
		manifest, manifestBytes, manifestDesc := manifestForIndexData(t, indexData, dataBytes)
