...
func BenchmarkOCIFlow(b *testing.B) {
	...
	for _, bc := range cases {
		b.Run(bc.name, func(b *testing.B) {
			...
			repo := fmt.Sprintf("bench/%s", sanitizeBenchName(bc.name))
...
func BenchmarkOCIPullCache(b *testing.B) {
	...
	for _, bc := range cases {
		b.Run(bc.name, func(b *testing.B) {
			data := createBenchOCIData(b, bc.fileCount, bc.fileSize, bc.compression, bc.pattern)
			repo := fmt.Sprintf("bench-cache/%s", sanitizeBenchName(bc.name))
...
func startRegistry(tb testing.TB, ctx context.Context) string {
	tb.Helper()
