...
func (c *RefCache) GetDigest(ref string) (dgst string, ok bool) {
	path := c.path(ref)
...
func (c *RefCache) PutDigest(ref, dgst string) error {
	path := c.path(ref)
	root, err := os.OpenRoot(c.dir)
	if err != nil {
	...
	if _, statErr := root.Stat(path); statErr == nil {
		...
		return fmt.Errorf("stat cache entry: %w", statErr)
	}

	written := int64(len(dgst))
	hasCapacity, capacityErr := c.ensureCapacity(written)
	if capacityErr != nil {
		return capacityErr
	}
	if !hasCapacity {
	...
	if dir != "." {
		if mkdirErr := root.MkdirAll(dir, c.dirPerm); mkdirErr != nil {
	...
	if _, err := tmp.WriteString(dgst); err != nil {
		_ = tmp.Close()          //nolint:errcheck // best-effort cleanup
		_ = root.Remove(tmpPath) //nolint:errcheck // best-effort cleanup
...
func isValidDigestFormat(s string) bool {
	...
	for _, ch := range []byte(algo) {
		if !isValidAlgoChar(ch) {
			return false
		}
	}
	// Hex part must be valid hex characters
	for _, ch := range []byte(hexPart) {
		if !isHexChar(ch) {
			return false
		}
	}
...
func isValidAlgoChar(ch byte) bool {
	return (ch >= 'a' && ch <= 'z') ||
		(ch >= 'A' && ch <= 'Z') ||
		(ch >= '0' && ch <= '9') ||
		ch == '+' || ch == '-' || ch == '.' || ch == '_'
...
//
//nolint:gocyclo // complexity from file I/O error handling is acceptable
...
func (c *ManifestCache) path(dgst string) (string, error) {
	// Strip algorithm prefix (e.g., "sha256:") to get hex hash
	hexHash, err := sanitizeHexDigest(dgst)
	if err != nil {
		return "", err
	}
	if c.shardPrefixLen <= 0 {
...
func (c *IndexCache) path(dgst string) (string, error) {
	hexHash, err := sanitizeHexDigest(dgst)
	if err != nil {
		return "", err
	}
...
func sanitizeHexDigest(dgst string) (string, error) {
	hexHash := dgst
	if idx := strings.IndexByte(dgst, ':'); idx >= 0 {
		hexHash = dgst[idx+1:]
	}
	if hexHash == "" {
		return "", fmt.Errorf("invalid digest %q", dgst)
	}
	for _, ch := range []byte(hexHash) {
		switch {
		case ch >= '0' && ch <= '9':
		case ch >= 'a' && ch <= 'f':
		case ch >= 'A' && ch <= 'F':
